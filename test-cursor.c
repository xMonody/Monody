/* cursor-logic test: mimics a terminal (SSD toplevel + I-beam cursor request)
 * and drives a virtual pointer across the title strip / body / edge zones /
 * empty desktop while the compositor logs every cursor decision with
 * WLR_DEBUG ("cursor: all-scroll" etc). Verifies:
 *   - hovering the top strip picks the "all-scroll" cursor without pressing
 *   - leaving a window through left/right/bottom resets to the default
 *     arrow instead of restoring a stale client cursor (I-beam) */
#define _GNU_SOURCE
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
...
static struct wl_surface *cursor_surface; /* I-beam lookalike */
static struct wl_buffer *cursor_buffer;
static bool got_enter;
...
/* on enter, request an I-beam-ish cursor exactly like a terminal does */
...
static void create_window(void) {
	...
	xdg_toplevel_set_title(xdg_toplevel, "cursor-test");
...
/* tracked absolute position of the virtual pointer */
...
int main(void) {
	...
	/* park the cursor at the output center */
	...
	create_window(); /* 200x100 centered at (cx,cy) -> box (cx-100,cy-50)-(cx+100,cy+50) */
	printf("window box (%d,%d)-(%d,%d); title strip y in [%d,%d)\n",
		cx - 100, cy - 50, cx + 100, cy + 50, cy - 50, cy - 30);
	fflush(stdout);
	...
	say("left edge zone");
	move_to(cx - 110, cy);
	say("empty desktop left of zone");
	move_to(cx - 130, cy);
	say("bottom edge zone");
	move_to(cx, cy + 56);
	say("empty desktop below zone");
	move_to(cx, cy + 80);
	say("back into body");
	move_to(cx, cy);
	say("title strip again");
	move_to(cx, cy - 40);

	usleep(200000);
	printf("done (got_enter=%d)\n", got_enter);
	return 0;
}
